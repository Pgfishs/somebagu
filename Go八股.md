...
# 为什么用混合写屏障而不是读屏障
1. 减少保守扫描：垃圾回收器需要扫描堆上的对象，以确定哪些对象是可达的，哪些对象是可以回收的。使用混合写屏障可以减少对非指针字段的扫描，因为只有在写入指针字段时才会触发屏障。相比之下，使用读屏障需要对每个对象进行扫描，无论是否有指针字段被读取。
2. 减少内存访问：混合写屏障只在写入指针字段时触发，而不会在读取指针字段时触发。这样可以减少GC的开销，并且减少了对内存的访问次数，提高了执行效率。
3. 保持原子性：混合写屏障可以确保在写入指针字段时，相关的元数据信息（如类型信息）被正确更新。而读屏障无法提供这种保证，可能导致垃圾回收器无法正确地跟踪对象的状态。
# Go GC的流程
| 阶段               | 说明                            | 赋值器状态 |
| ---------------- | ----------------------------- | ----- |
